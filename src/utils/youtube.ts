...
function extractArtist(title: string): {title: string; artist: string} {
  ...
  const channelPatterns = [/^(.+?)\s*(?:VEVO|Records|Music|Official|Channel|Topic)$/gi]
...
export async function fetchYouTubePlaylist(
  ...
  try {
    ...
    const playlistData = (await playlistResponse.json()) as YouTubeListResponse<YouTubePlaylist>
    ...
    do {
      ...
      const itemsData = (await itemsResponse.json()) as YouTubeListResponse<YouTubePlaylistItem>
    ...
    for (let i = 0; i < videoIds.length; i += batchSize) {
      ...
      if (videosResponse.ok) {
        const videosData = (await videosResponse.json()) as YouTubeListResponse<YouTubeVideo>
    ...
    // Process tracks
    const tracks: YouTubeTrack[] = validItems.map(item => {
